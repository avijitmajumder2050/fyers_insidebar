...
def get_multiple_ltps(symbols: list[str]) -> dict:
    ...
    for item in resp["d"]:
        if item["s"] == "ok":
            ltp_map[item["n"]] = float(item["v"]["lp"])
...
def run_strategy() -> None:
    ...
    if candidates.empty:
        tg.notify_no_trade("Fewer than 3 candidates in CSV today.")
        return

    # ── Step 2: One trade per day guard ───────────────────────
    if s3_utils.has_trade_today():
        logger.info("Trade already exists for today. Exiting.")
        tg.notify_no_trade("A trade was already taken today.")
        return

    # ── Step 3–10: Cascade through ranked candidates ──────────
    for _, row in candidates.iterrows():
        raw_symbol     = str(row["stock_name"]).strip()
        fyers_symbol   = to_fyers_symbol(raw_symbol)
        csv_sl         = float(row["sl"])

        logger.info("─── Evaluating: %s (%s) ───", raw_symbol, fyers_symbol)

        # ── Live validation ────────────────────────────────────
        try:
            ltp = get_ltp(fyers_symbol)
        except Exception as exc:
            logger.error("LTP fetch failed for %s: %s", fyers_symbol, exc)
            tg.notify_rejection(raw_symbol, f"LTP fetch error: {exc}")
            continue

        actual_sl_pct = calc_actual_sl_pct(ltp, csv_sl)
        logger.info("%s  LTP=%.2f  SL=%.2f  SL%%=%.2f", raw_symbol, ltp, csv_sl, actual_sl_pct)

        if actual_sl_pct > MAX_SL_PCT:
            reason = f"SL% {actual_sl_pct:.2f}% exceeds max {MAX_SL_PCT}%"
            logger.info("Rejected %s — %s", raw_symbol, reason)
            tg.notify_rejection(raw_symbol, reason)
            continue

        # ── Position sizing ────────────────────────────────────
        try:
            qty = calculate_qty(ltp, csv_sl)
        except ValueError as exc:
            tg.notify_rejection(raw_symbol, str(exc))
            continue

        if qty <= 0:
            tg.notify_rejection(raw_symbol, "Calculated qty is 0 — insufficient funds or too wide SL.")
            continue

        logger.info("Position: qty=%d  ltp=₹%.2f  sl=₹%.2f", qty, ltp, csv_sl)

        # ── Place MARKET BUY ───────────────────────────────────
        try:
            buy_order_id = place_market_buy(fyers_symbol, qty)
            entry_price  = get_order_fill_price(buy_order_id)
        except Exception as exc:
            logger.error("Buy order failed for %s: %s", fyers_symbol, exc)
            tg.notify_rejection(raw_symbol, f"Order failed: {exc}")
            continue

        logger.info("Filled: %s  entry=₹%.2f  qty=%d", raw_symbol, entry_price, qty)
        tg.notify_trade_entry(raw_symbol, entry_price, csv_sl, qty, actual_sl_pct)

        # Save initial journal entry (status=OPEN)
        s3_utils.save_trade({
            "trade_date":  __import__("datetime").date.today().isoformat(),
            "symbol":      raw_symbol,
        ...
        try:
            sl_order_id = _place_sl_order(fyers_symbol, qty, csv_sl)
        except Exception as exc:
            logger.error("Could not place SL order for %s: %s", fyers_symbol, exc)
            tg.send(f"⚠️ WARNING: Entry filled for {raw_symbol} but SL order FAILED. Manual action required!")
        ...
        # One successful entry — done for the day
        logger.info("Session complete. Trade managed and closed for %s.", raw_symbol)
        return

    logger.info("All candidates exhausted without a successful entry.")
